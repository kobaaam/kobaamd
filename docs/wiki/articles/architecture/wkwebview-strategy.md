...
tags: [wkwebview, mermaid, d2, markdown-preview, memory, performance, security, csp]
...
### プレビュー硬化（2026-07-02 セキュリティ対応、PR #167）
<!-- llm-context: KMD-28（WKWebView XSS）への対応として 2026-07-02 に実施したプレビュー硬化の設計判断。MarkdownWebView / WorkspacePreviewHTTPServer / ChromiumPreviewController が対象。 -->

2026-07-02 のセキュリティ監査（KMD-28）で、生 HTML パススルー・CSP 不在・`javascript:` リンク許可・Mermaid `loose` モードの連鎖が XSS の悪用経路になり得ると判明した。以下の多層防御を PR #167 で実施した。
...
**残課題（バックログ）**:
- `HTMLPreviewView`（Rendered タブ）で生 HTML をユーザーファイルからそのまま Chromium に渡している点。許可リストベースのサニタイズは未実装のままバックログに積まれている。
...
- [[security-hardening]] — KMD-28 プレビュー硬化の全体セキュリティ方針（PR #167 の背景）
